/*
 * iqperf, Copyright (c) 2014, 2015, 2016, 2017, The Regents of the University of
 ...
 */
/* iqperf_server_api.c: Functions to be used by an iqperf server
*/
...
int
iqperf_server_listen(struct iqperf_test *test)
{
    retry:
    ...
    if (!test->json_output) {
	iqperf_printf(test, "-----------------------------------------------------------\n");
	iqperf_printf(test, "Server listening on %d\n", test->server_port);
    }
...
int
iqperf_accept(struct iqperf_test *test)
{
    int s;
	...
	if (iqperf_set_send_state(test, PARAM_EXCHANGE) != 0)
            return -1;
        if (iqperf_exchange_parameters(test) < 0)
            return -1;
	if (test->server_affinity != -1) 
	    if (iqperf_setaffinity(test, test->server_affinity) != 0)
		return -1;
        if (test->on_connect)
...
/**************************************************************************/
int
iqperf_handle_message_server(struct iqperf_test *test)
{
    int rval;
    ...
    if ((rval = Nread(test->ctrl_sck, (char*) &test->state, sizeof(signed char), Ptcp)) <= 0) {
        if (rval == 0) {
	    iqperf_err(test, "the client has unexpectedly closed the connection");
            i_errno = IECTRLCLOSE;
            test->state = iqperf_DONE;
            return 0;
        } else {
    ...
    switch(test->state) {
	    ...
	    if (iqperf_set_send_state(test, EXCHANGE_RESULTS) != 0)
                return -1;
            if (iqperf_exchange_results(test) < 0)
                return -1;
	    if (iqperf_set_send_state(test, DISPLAY_RESULTS) != 0)
                return -1;
	    ...
	    iqperf_err(test, "the client has terminated");
            SLIST_FOREACH(sp, &test->streams, streams) {
                FD_CLR(sp->socket, &test->read_set);
                FD_CLR(sp->socket, &test->write_set);
                close(sp->socket);
...
{
    struct iqperf_test *test = client_data.p;
    struct iqperf_stream *sp;
...
static int
create_server_timers(struct iqperf_test * test)
{
    struct timeval now;
    TimerClientData cd;
...
{   
    ...
    test->omitting = 0;
    iqperf_reset_stats(test);
    if (test->verbose && !test->json_output && test->reporter_interval == 0)
	iqperf_printf(test, "%s", report_omit_done);
...
static int
create_server_omit_timer(struct iqperf_test * test)
{
    struct timeval now;
    TimerClientData cd; 
...
static void
cleanup_server(struct iqperf_test *test)
{
...
int
iqperf_run_server(struct iqperf_test *test)
{
    ...
    if (test->affinity != -1) 
	if (iqperf_setaffinity(test, test->affinity) != 0)
	    return -2;

    if (test->json_output)
	if (iqperf_json_start(test) < 0)
	    return -2;
    ...
    if (iqperf_server_listen(test) < 0) {
        return -2;
    }
    ...
    test->state = iqperf_START;
    streams_accepted = 0;

    while (test->state != iqperf_DONE) {
	...
	if (result > 0) {
            ...
            if (FD_ISSET(test->ctrl_sck, &read_set)) {
                if (iqperf_handle_message_server(test) < 0) {
		    cleanup_server(test);
                    return -1;
		}
		    ...
		    if (iqperf_set_send_state(test, TEST_START) != 0) {
			cleanup_server(test);
                        return -1;
		    }
                    if (iqperf_init_test(test) < 0) {
			cleanup_server(test);
                        return -1;
		    }
		    ...
		    if (test->reverse)
			if (iqperf_create_send_timers(test) < 0) {
			    cleanup_server(test);
			    return -1;
			}
		    if (iqperf_set_send_state(test, TEST_RUNNING) != 0) {
			cleanup_server(test);
                        return -1;
		    }
                }
            ...
            if (test->state == TEST_RUNNING) {
                if (test->reverse) {
                    ...
                    if (iqperf_send(test, &write_set) < 0) {
			cleanup_server(test);
                        return -1;
		    }
                } else {
                    // Regular mode. Server receives.
                    if (iqperf_recv(test, &read_set) < 0) {
			cleanup_server(test);
                        return -1;
		    }
                }
    ...
    if (test->server_affinity != -1) 
	if (iqperf_clearaffinity(test) != 0)
	    return -1;
