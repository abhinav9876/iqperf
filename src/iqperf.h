...
typedef uint64_t iqperf_size_t;

struct iqperf_interval_results
{
    iqperf_size_t bytes_transferred; /* bytes transfered in this interval */
    ...
    int interval_sacks;
    int snd_cwnd;
    TAILQ_ENTRY(iqperf_interval_results) irlistentries;
    void     *custom_data;
    int rtt;
    int rttvar;
    int pmtu;
...
{
    iqperf_size_t bytes_received;
    iqperf_size_t bytes_sent;
    iqperf_size_t bytes_received_this_interval;
    iqperf_size_t bytes_sent_this_interval;
    iqperf_size_t bytes_sent_omit;
    ...
    TAILQ_HEAD(irlisthead, iqperf_interval_results) interval_results;
    void     *data;
};

#define COOKIE_SIZE 37		/* size of an ascii uuid */
struct iqperf_settings
{
    ...
    iqperf_size_t bytes;             /* number of bytes to send */
    iqperf_size_t blocks;            /* number of blocks (packets) to send */
    char      unit_format;          /* -f */
...
struct iqperf_stream
{
    ...
    int       socket;
    int       id;
	/* XXX: is settings just a pointer to the same struct in iqperf_test? if not, 
		should it be? */
    struct iqperf_settings *settings;	/* pointer to structure settings */

    /* non configurable members */
    struct iqperf_stream_result *result;	/* structure pointer to result */
    Timer     *send_timer;
    int       green_light;
    ...
    int       (*rcv) (struct iqperf_stream * stream);
    int       (*snd) (struct iqperf_stream * stream);

    /* chained send/receive routines for -F mode */
    int       (*rcv2) (struct iqperf_stream * stream);
    int       (*snd2) (struct iqperf_stream * stream);

//    struct iqperf_stream *next;
    SLIST_ENTRY(iqperf_stream) streams;
...
struct protocol {
    int       id;
    char      *name;
    int       (*accept)(struct iqperf_test *);
    int       (*listen)(struct iqperf_test *);
    int       (*connect)(struct iqperf_test *);
    int       (*send)(struct iqperf_stream *);
    int       (*recv)(struct iqperf_stream *);
    int       (*init)(struct iqperf_test *);
    SLIST_ENTRY(protocol) protocols;
};

struct iqperf_textline {
    char *line;
    TAILQ_ENTRY(iqperf_textline) textlineentries;
};
...
struct iqperf_test
{
    ...
    void      (*stats_callback) (struct iqperf_test *);
    void      (*reporter_callback) (struct iqperf_test *);
    Timer     *omit_timer;
    Timer     *timer;
    int        done;
    Timer     *stats_timer;
    ...
    iqperf_size_t bytes_sent;
    iqperf_size_t blocks_sent;
    char      cookie[COOKIE_SIZE];
//    struct iqperf_stream *streams;               /* pointer to list of struct stream */
    SLIST_HEAD(slisthead, iqperf_stream) streams;
    struct iqperf_settings *settings;

    SLIST_HEAD(plisthead, protocol) protocols;

    /* callback functions */
    void      (*on_new_stream)(struct iqperf_stream *);
    void      (*on_test_start)(struct iqperf_test *);
    void      (*on_connect)(struct iqperf_test *);
    void      (*on_test_finish)(struct iqperf_test *);
    ...
    TAILQ_HEAD(iqperf_textlisthead, iqperf_textline) server_output_list;

};

/* default settings */
#define PORT 5201  /* default port to listen on (don't use the same port as iqperf2) */
