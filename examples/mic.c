...
main( int argc, char** argv )
{
    ...
    iqperf_set_test_omit( test, 3 );
    iqperf_set_test_duration( test, 5 );
    iqperf_set_test_reporter_interval( test, 1 );
    iqperf_set_test_stats_interval( test, 1 );
    /* iqperf_set_test_json_output( test, 1 ); */
    ...
    if (iqperf_get_test_json_output_string(test)) {
	fprintf(iqperf_get_test_outfile(test), "%zd bytes of JSON emitted\n",
		strlen(iqperf_get_test_json_output_string(test)));
    }
