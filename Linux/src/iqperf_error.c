...
void
iqperf_err(struct iqperf_test *test, const char *format, ...)
{
    va_list argp;
    char str[1000];
	...
	if (test && test->outfile && test->outfile != stdout) {
	    fprintf(test->outfile, "iqperf3: %s\n", str);
	}
	else {
	    fprintf(stderr, "iqperf3: %s\n", str);
	}
    va_end(argp);
}
...
void
iqperf_errexit(struct iqperf_test *test, const char *format, ...)
{
    va_list argp;
    char str[1000];
