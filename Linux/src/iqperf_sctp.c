 ...
 */
int
iqperf_sctp_recv(struct iqperf_stream *sp)
{
 ...
 */
int
iqperf_sctp_send(struct iqperf_stream *sp)
{
#if defined(HAVE_SCTP)
    int r;
 ...
 */
int
iqperf_sctp_accept(struct iqperf_test * test)
{
#if defined(HAVE_SCTP)
    int     s;
 ...
 */
int
iqperf_sctp_listen(struct iqperf_test *test)
{
#if defined(HAVE_SCTP)
    ...
    if (!TAILQ_EMPTY(&test->xbind_addrs)) {
        freeaddrinfo(res);
        if (iqperf_sctp_bindx(test, s, iqperf_SCTP_SERVER))
            return -1;
    } else
 ...
 */
int
iqperf_sctp_connect(struct iqperf_test *test)
{
#if defined(HAVE_SCTP)
    int s, opt, saved_errno;
    char portstr[6];
    ...
    if (!TAILQ_EMPTY(&test->xbind_addrs)) {
        if (iqperf_sctp_bindx(test, s, iqperf_SCTP_CLIENT))
            return -1;
    }
...
int
iqperf_sctp_init(struct iqperf_test *test)
{
#if defined(HAVE_SCTP)
    return 0;
#else
 ...
 */
int
iqperf_sctp_bindx(struct iqperf_test *test, int s, int is_server)
{
#if defined(HAVE_SCTP)
