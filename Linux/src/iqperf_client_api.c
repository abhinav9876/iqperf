...
int
iqperf_create_streams(struct iqperf_test *test)
{
    int i, s;
#if defined(HAVE_TCP_CONGESTION)
    int saved_errno;
...
static int
create_client_timers(struct iqperf_test * test)
{
    struct timeval now;
    TimerClientData cd;
...
static int
create_client_omit_timer(struct iqperf_test * test)
{
    struct timeval now;
    TimerClientData cd;
...
int
iqperf_handle_message_client(struct iqperf_test *test)
{
    int rval;
    int32_t err;
    ...
    switch (test->state) {
        case PARAM_EXCHANGE:
            ...
            break;
        case CREATE_STREAMS:
            if (iqperf_create_streams(test) < 0)
                return -1;
            break;
        case TEST_START:
...
/* iqperf_connect -- client to server connection function */
int
iqperf_connect(struct iqperf_test *test)
{
...
int
iqperf_client_end(struct iqperf_test *test)
{
    struct iqperf_stream *sp;
    ...
    if (iqperf_set_send_state(test, iqperf_DONE) != 0)
        return -1;
...
int
iqperf_run_client(struct iqperf_test * test)
{
    int startup;
    int result = 0;
	...
	if (result > 0) {
	    if (FD_ISSET(test->ctrl_sck, &read_set)) {
 	        if (iqperf_handle_message_client(test) < 0) {
		    return -1;
		}
	...
	if (test->state == TEST_RUNNING) {
		...
		cpu_util(test->cpu_util);
		test->stats_callback(test);
		if (iqperf_set_send_state(test, TEST_END) != 0)
		    return -1;
	    }
	}
	...
	iqperf_printf(test, "\n");
	iqperf_printf(test, "%s", report_done);
    }
