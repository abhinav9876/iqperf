/*
 * iqperf, Copyright (c) 2014, 2017, The Regents of the University of
 ...
 * OSX has no support.
 *
 * I think MS Windows does support TCP_INFO, but iqperf3 does not currently support Windows.
 */
...
/*************************************************************/
void
save_tcpinfo(struct iqperf_stream *sp, struct iqperf_interval_results *irp)
{
#if (defined(linux) || defined(__FreeBSD__) || defined(__NetBSD__)) && \
	defined(TCP_INFO)
	...
	iqperf_err(sp->test, "getsockopt - %s", strerror(errno));
...
/*************************************************************/
long
get_total_retransmits(struct iqperf_interval_results *irp)
{
#if defined(linux) && defined(TCP_MD5SIG)
 ...
 */
long
get_snd_cwnd(struct iqperf_interval_results *irp)
{
#if defined(linux) && defined(TCP_MD5SIG)
 ...
 */
long
get_rtt(struct iqperf_interval_results *irp)
{
#if defined(linux) && defined(TCP_MD5SIG)
 ...
 */
long
get_rttvar(struct iqperf_interval_results *irp)
{
#if defined(linux) && defined(TCP_MD5SIG)
 ...
 */
long
get_pmtu(struct iqperf_interval_results *irp)
{
#if defined(linux) && defined(TCP_MD5SIG)
...
/*************************************************************/
void
build_tcpinfo_message(struct iqperf_interval_results *r, char *message)
{
#if defined(linux)
