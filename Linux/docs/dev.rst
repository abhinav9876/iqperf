...
The developer list for iqperf3 is:  iqperf-dev@googlegroups.com.
Information on joining the mailing list can be found at:

http://groups.google.com/group/iqperf-dev
...
Then submit to the iqperf3 issue tracker on GitHub:

https://github.com/esnet/iqperf/issues

**Note:** Issues submitted to the old iqperf3 issue tracker on Google
...
for a complete list of iqperf3 options)::
...
* The ``-Z`` flag sometimes causes the iqperf3 client to hang on OSX.
  (Issue #129)
...
* When specifying the TCP buffer size using the ``-w`` flag on Linux,
  ...
  iqperf3's ``--debug`` flag.)  However, CWND does not actually ramp up
...
* On some platforms (observed on at least one version of Ubuntu
  ...
  doing a ``make install`` before the ``iqperf3`` executable can find
...
iqperf3 version numbers use (roughly) a `Semantic Versioning
...
3. Preferably starting from a clean source tree (be sure that ``git
   ...
   the new version, such as bumping version numbers::
    ...
    vi src/iqperf3.1    # update manpage revision date if needed
    vi src/libiqperf.3  # update manpage revision date if needed
...
11. Send the PGP-signed release announcement to the following
    ...
    * iqperf-users@lists.sourceforge.net
...
12. Update the iqperf3 Project News section of the documentation site
...
13. If an update to the on-line manual page is needed, it can be
    ...
    import the result into ``invoking.rst``::
     ...
     TERM=
     export TERM
     nroff -Tascii -c -man src/iqperf3.1 | ul | sed 's/^/   /' > iqperf3.txt

Code Authors
------------

The main authors of iqperf3 are (in alphabetical order):  Jon Dugan,
...
iqperf3 contains some original code from iqperf2.  The authors of iqperf2
