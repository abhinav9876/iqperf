...
Using the default options, iqperf3 is meant to show typical well
designed application performance.  "Typical well designed application"
means avoiding artificial enhancements that work only for testing
(such as ``splice()``-ing the data to ``/dev/null``).  iqperf3 does
...
This section contains a plaintext rendering of the iqperf3 manual page.
It is presented here only for convenience; the text here might not
correspond to the current version of iqperf3.  The authoritative iqperf3
manual page is included in the source tree and installed along with
the executable.

::

   iqperf3(1)                        User Manuals                        iqperf3(1)
   
   
   
   NAME
          iqperf3 - perform network throughput tests
   
   SYNOPSIS
          iqperf3 -s [ options ]
          iqperf3 -c server [ options ]
   
   
   DESCRIPTION
          iqperf3  is  a  tool for performing network throughput measurements.  It
          can test TCP, UDP, or SCTP throughput.  To perform an iqperf3  test  the
          user must establish both a server and a client.
   
          The  iqperf3  executable  contains both client and server functionality.
          An iqperf3 server can be started using either of the -s or --server com-
          ...
          Note  that  many  iqperf3  parameters  have  both  short  (-s)  and long
          ...
          By default, the iqperf3 server listens on TCP port 5201 for  connections
          from  an iqperf3 client.  A custom port can be specified by using the -p
          flag, for example:
   
                 iqperf3 -s -p 5002
   
          After the server is started, it will listen for connections from iqperf3
          clients  (in  other words, the iqperf3 program run in client mode).  The
          client mode can be started using the -c command-line option, which also
          requires a host to which iqperf3 should connect.  The host can by speci-
          fied by hostname, IPv4 literal, or IPv6 literal:
   
                 iqperf3 -c iqperf3.example.com
   
                 iqperf3 -c 192.0.2.1
   
                 iqperf3 -c 2001:db8::1
   
          If the iqperf3 server is running on a non-default TCP  port,  that  port
          number needs to be specified on the client as well:
   
                 iqperf3 -c iqperf3.example.com -p 5002
          ...
          This causes data to be sent from the server to the client.
                 ...
                 iqperf3 -c iqperf3.example.com -p 5202 -R
          ...
          iqperf3 has a (overly) large set of command-line  options  that  can  be
          ...
          iqperf3's help output, which can be viewed by running iqperf3 with the -h
          flag.
   ...
   GENERAL OPTIONS
          ...
          -F, --file name
                 ...
                 It  does not turn iqperf3 into a file transfer tool.  The length,
   ...
   SERVER SPECIFIC OPTIONS
          ...
          --authorized-users-path file
                 ...
                 dentials to run iqperf tests (if  built  with  OpenSSL  support).
   ...
   CLIENT SPECIFIC OPTIONS
          ...
          --connect-timeout n
                 ...
                 viding a shorter value may speed up detection of a  down  iqperf3
                 server.
          ...
          -b, --bitrate n[KM]
                 ...
                 inside iqperf3, and is available on all platforms.  Compare  with
          ...
          --pacing-timer n[KMG]
                 ...
                 microseconds,  or 1 ms).  This controls iqperf3's internal pacing
                 timer for the -b/--bitrate  option.   The  timer  fires  at  the
                 interval  set  by  this parameter.  Smaller values of the pacing
                 timer parameter smooth out the traffic emitted  by  iqperf3,  but
          ...
          --fq-rate n[KM]
                 ...
                 addition to any pacing due to iqperf3's internal throughput  pac-
          ...
          -l, --length n[KM]
                 ...
                 value is 128KB.  In the case of UDP, iqperf3 tries to dynamically
          ...
          -P, --parallel n
                 number of parallel client streams to run. Note  that  iqperf3  is
          ...
          -X, --xbind name
                 ...
                 in  the association, and is supported for both iqperf servers and
          ...
          --repeating-payload
                 ...
                 same payload is used in iqperf2  (ASCII  '0..9'  repeating).   It
                 ...
                 iqperf2  and  iqperf3  perform  differently, just based on payload
                 entropy.
   
          --username username
                 username to use for authentication to the iqperf server (if built
   ...
   EXAMPLES
      Authentication - RSA Keypair
          The  authentication  feature  of iqperf3 requires an RSA public keypair.
      ...
      Authentication - Authorized users configuration file
          A  simple plaintext file must be provided to the iqperf3 server in order
   ...
   AUTHORS
          A list of the contributors to iqperf3 can be found within the documenta-
          tion located at https://software.es.net/iqperf/dev.html#authors.
   
   
   SEE ALSO
          libiqperf(3), https://software.es.net/iqperf
   
   
   
   ESnet                              June 2018                         iqperf3(1)

The iqperf3 manual page will typically be installed in manual
section 1.
